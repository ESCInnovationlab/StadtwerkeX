...
.metric-detail { font-size: 10px; color: #cbd5e1; margin-top: 3px; }
...
.user-msg { background: #1e293b; color: #ffffff; padding: 12px; border-radius: 12px; margin-bottom: 10px; text-align: right; font-size: 14px; }
...
if not st.session_state.authenticated:
    st.markdown('<h1 style="color:#0f172a; text-align:center; margin-top:100px; white-space: nowrap;">🏢 STADTWERKE WÜLFRATH</h1><p style="text-align:center; color:#64748b;">Plattform für Infrastruktur-Intelligenz</p>', unsafe_allow_html=True)
...
st.markdown('<h1 class="main-header">STADTWERKE WÜLFRATH</h1>', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if st.session_state.drilldown_type != "None":
        ...
        if not view_df.empty:
            ...
            # Add Netz-Karte info (Material + Dimension)
            if "Material Netzleitung" in view_df.columns and "Dimension Netzleitung" in view_df.columns:
                view_df["Netz-Karte"] = view_df["Material Netzleitung"].astype(str) + " / " + view_df["Dimension Netzleitung"].astype(str)
            else:
                view_df["Netz-Karte"] = "N/A"

            st.data_editor(
                view_df[["Kundennummer", "Sparte", "Straße", "Hausnummer", "Werkstoff", "Alter", "Risiko", "Einbaujahr", "Netz-Karte"]],
                column_config={
...
with tabs[1]:
    ...
    if not map_df.empty:
        ...
        with c_list:
            ...
            # Add Netz-Karte info
            if "Material Netzleitung" in map_df.columns and "Dimension Netzleitung" in map_df.columns:
                map_df["Netz-Karte"] = map_df["Material Netzleitung"].astype(str) + " / " + map_df["Dimension Netzleitung"].astype(str)
            else:
                 map_df["Netz-Karte"] = "N/A"

            st.data_editor(
                map_df[["Kundennummer", "Sparte", "Straße", "Hausnummer", "Risiko", "Alter", "Netz-Karte"]],
                column_config={
                    "Kundennummer": st.column_config.TextColumn("ID", width="small"),
                    "Risiko": st.column_config.SelectboxColumn("Risiko", options=["Hoch", "Mittel", "Niedrig", "Unbekannt"]),
...
with tabs[2]:
    ...
    st.dataframe(df[df["Dokumente"] == "Lückenhaft"], use_container_width=True)
